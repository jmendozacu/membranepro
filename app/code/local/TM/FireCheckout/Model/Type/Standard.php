...
//            }
     ...
     * @return  Mage_Checkout_Model_Type_Onepage
     */
    public function saveBilling($data, $customerAddressId, $validate = true, $updatePfpj = true)
    {
        if (empty($data)) {
            return array('error' => -1, 'message' => $this->_helper->__('Invalid data.'));
        }
        ...
        $referer = parse_url(Mage::helper('core/http')->getHttpReferer());
        ...
        if (!empty($customerAddressId)) {
            ...
            if ($customerAddress->getId()) {
                ...
                /*
                 * added as an update by BGB's team
                 */
                //$referer = parse_url(Mage::helper('core/http')->getHttpReferer());
                
                if (strpos($referer['path'], 'firecheckout') !== false && $updatePfpj) {
                    //die('if update');
                    if ($data['pj_billing']) {
                        $pfpjForBilling = '1';
                        $pfpjTipPers = '2';
                    ...
                    $customerAddress->setData('company', $data['company']);
                    $customerAddress->setData('pfpj_cui', $data['pfpj_cui']);
                    $customerAddress->setData('pfpj_reg_com', $data['pfpj_reg_com']);
                    $customerAddress->setData('pfpj_banca', $data['pfpj_banca']);
                    $customerAddress->setData('pfpj_iban', $data['pfpj_iban']);
                    $customerAddress->setData('pfpj_cnp', $data['pfpj_cnp']);
                    $customerAddress->setData('pfpj_serienr_buletin', $data['pfpj_serienr_buletin']);
                    $customerAddress->setData('pfpj_for_billing', $pfpjForBilling);
                    $customerAddress->setData('pfpj_tip_pers', $pfpjTipPers);
            ...
            if (strpos($referer['path'], 'firecheckout') !== false && $updatePfpj) {
                ...
                $address->setData('company', $data['company']);
                $address->setData('pfpj_cui', $data['pfpj_cui']);
                $address->setData('pfpj_reg_com', $data['pfpj_reg_com']);
                $address->setData('pfpj_banca', $data['pfpj_banca']);
                $address->setData('pfpj_iban', $data['pfpj_iban']);
                $address->setData('pfpj_cnp', $data['pfpj_cnp']);
                $address->setData('pfpj_serienr_buletin', $data['pfpj_serienr_buletin']);
                $address->setData('pfpj_for_billing', $pfpjForBilling);
                $address->setData('pfpj_tip_pers', $pfpjTipPers);
                $address->save();
            }
            /*
             * end BGB's update
