...
class Webtex_CustomerPrices_Model_Prices extends Mage_Core_Model_Abstract {
    ...
    public function getProductPrice($product)
    {
        ...
        $query = $connection->select()->from($tablePrefix . 'customerprices_prices')
                            ->where('product_id = ' . $product->getId() . ' AND customer_id = ' . $customerId);
    ...
    public function getProductCustomerPrice($product,$customerId)
    {
        $connection = Mage::getSingleton('core/resource')->getConnection('core_write');
        $tablePrefix = (string) Mage::getConfig()->getTablePrefix();
    ...
    public function getProductCustomerSpecialPrice($product,$customerId)
    {
        $connection = Mage::getSingleton('core/resource')->getConnection('core_write');
        $tablePrefix = (string) Mage::getConfig()->getTablePrefix();
    ...
    public function loadByCustomer($productId, $customerId)
    {
        $this->setData($this->getResource()->loadByCustomer($productId, $customerId));
        return $this;
    }
