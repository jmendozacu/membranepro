...
class Magestore_Dailydeal_Adminhtml_DailydealController extends Mage_Adminhtml_Controller_Action
{
		...
		if(!is_array($dailydealIds)) {
			...
			try {
				...
				$this->_getSession()->addSuccess(
					$this->__('Total of %d Deal(s) were successfully updated', count($dailydealIds))
				);
			} catch (Exception $e) {
	...
	{
        $this->loadLayout();
        $this->getLayout()->getBlock('dailydeal.edit.tab.listproduct')
