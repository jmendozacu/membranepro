...
class Mage_Page_Block_Html_Topmenu_Renderer extends Mage_Page_Block_Html_Topmenu
{
    ...
    function getCategoryImage($img, $width, $height = false)
    {
        if(!$height) $height = $width;
        
        $mediaDir = Mage::getBaseDir('media') . DS . 'catalog' . DS . 'category' . DS;
        $cacheDir = $mediaDir . 'cache' . DS . $width . 'x' .$height . DS;
                  
        if(file_exists($mediaDir . $img)){
             
             if(!is_dir($cacheDir)){
                  mkdir($cacheDir);
             }
             
             $_image = new Varien_Image($mediaDir . $img);
             $_image->constrainOnly(true);
             $_image->keepAspectRatio(true);
             $_image->keepFrame(true);
             $_image->keepTransparency(true);
             $_image->resize($width, $height);
             $_image->save($cacheDir . $img);
             
             return Mage::getBaseUrl('media') . 'catalog' . DS . 'category' . DS . 'cache' . DS . $width . 'x' .$height . $img;
         }
