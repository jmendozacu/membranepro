	...
	public function startJs(){
		...
		if($va_Slides){
			foreach ($va_Slides as $vo_Slide){
				if($this->getModel()->getShowNameInButtons() == 1)
					$vs_Js .= 'interaktingslider.addSlideWithTitle("'.$vo_Slide->getFormatedContent().'","'.$vo_Slide->getName().'"); ';
				else
					$vs_Js .= 'interaktingslider.addSlide("'.$vo_Slide->getFormatedContent().'","'.$vo_Slide->getName().'"); ';
			}
		}
		$vs_Js .= "
