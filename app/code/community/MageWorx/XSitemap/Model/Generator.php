...
class MageWorx_XSitemap_Model_Generator
{
    ...
    protected function _generateXmlFromCategories()
    {
        ...
        if(!empty($altCodes)){
            $alternateUrlsCollection = Mage::getResourceModel('seosuite/alternate_catalog_category')->getAllCategoryUrls(array_keys($altCodes));
        }
        ...
        foreach ($collection as $item) {
            ...
            $this->_xmlWriter->write($this->_helper->trailingSlash($url), $lastmod, $changefreq, $priority, false, $alternateUrls);
        }
    ...
    protected function _generateXmlFromProducts()
    {
        ...
        if ($this->_counter < $this->_totalProduct) {
            ...
            if(!empty($altCodes)){
                ...
                $alternateUrlsCollection = Mage::getResourceModel('seosuite/alternate_catalog_product')->getAllProductUrls(array_keys($altCodes), $arrayTargetPath);
            }
            ...
            foreach ($collection as $item) {
                ...
                $this->_xmlWriter->write($this->_helper->trailingSlash($url), $lastmod, $changefreq, $priority, $imageUrl, $alternateUrls);
            }
            unset($collection);
        }
    }
    ...
    protected function _generateXmlFromCms()
    {
        ...
        foreach ($collection as $item) {
            ...
            if(!empty($altCodes)){
                $alternateUrlsCollection = Mage::getResourceModel('seosuite/alternate_cms_page')
