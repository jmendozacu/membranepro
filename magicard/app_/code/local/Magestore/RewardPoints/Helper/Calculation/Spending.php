 ...
 */
class Magestore_RewardPoints_Helper_Calculation_Spending
    extends Magestore_RewardPoints_Helper_Calculation_Abstract
{
    ...
    public function getQuoteBaseTotal($quote, $address = null)
    {
        ...
        $baseTotal = $address->getBaseSubtotal() + $address->getBaseDiscountAmount()
                    - $this->getPointItemDiscount();
        $applyTaxAfterDiscount = (bool)Mage::getStoreConfig(
            Mage_Tax_Model_Config::CONFIG_XML_PATH_APPLY_AFTER_DISCOUNT,
            $quote->getStoreId()
        );
        if (Mage::getStoreConfig(self::XML_PATH_SPEND_FOR_TAX, $quote->getStoreId())
        ...
        if (Mage::getStoreConfig(self::XML_PATH_SPEND_FOR_SHIPPING_TAX, $quote->getStoreId())
            && !$applyTaxAfterDiscount
        ) {
    ...
    public function getSliderRuleDiscount()
    {
        ...
        if (is_array($rewardSalesRules) && isset($rewardSalesRules['base_discount'])
            && $session->getData('use_point')
        ) {
    ...
    public function getSliderRulePoint()
    {
        $session = Mage::getSingleton('checkout/session');
        $rewardSalesRules = $session->getRewardSalesRules();
        if (is_array($rewardSalesRules) && isset($rewardSalesRules['use_point'])
            && $session->getData('use_point')
        ) {
    ...
    public function getSpendingRateAsRule()
    {
        ...
        $rate = Mage::getSingleton('rewardpoints/rate')->getRate(
            Magestore_RewardPoints_Model_Rate::POINT_TO_MONEY,
    ...
    public function getRuleMaxPointsForQuote($rule, $quote)
    {
        ...
        if ($rule->getId() == 'rate') {
            if ($rule->getBaseRate() && $rule->getPointsSpended()) {
                ...
                if($quoteTotal > $maxPriceSpend && $maxPriceSpend > 0) $quoteTotal = $maxPriceSpend;
                //End Hai.Tran 13/11/2013 add limit spend theo quote total
                
                $maxPoints = ceil(($quoteTotal - $this->getCheckedRuleDiscount())
                        / $rule->getBaseRate()
