    ...
    public function getOrderPlaceRedirectUrl ()
    {
        ...
        try
        {
            ...
            if (! empty($company))
            {
                ...
                $billingAddress->fiscalNumber = $billing->getPfpjCui();
                $billingAddress->identityNumber = $billing->getPfpjRegCom();
                $billingAddress->bank = $billing->getPfpjBanca();
                $billingAddress->iban = $billing->getPfpjIban();
            } else
            {
            ...
            $billingAddress->county = $billingAddress->state;
            ...
            if (! empty($company))
            {
                ...
                $shippingAddress->fiscalNumber = $shipping->getPfpjCui();
                $shippingAddress->identityNumber = $shipping->getPfpjRegCom();
                $shippingAddress->bank = $shipping->getPfpjBanca();
                $shippingAddress->iban = $shipping->getPfpjIban();
            } else
            {
            ...
            $shippingAddress->county = $shippingAddress->state;
            ...
            // convert price CURRENCY from EUR to RON
	        $objPmReqCard->invoice->currency = self::chargingCurrency;
	        // convert price VALUE from base currency to current currency
	        $objPmReqCard->invoice->amount = Mage::helper('directory')->currencyConvert($objPmReqCard->invoice->amount, self::baseCurrency, self::chargingCurrency);
            //print_r($objPmReqCard);die();
...
$this->_order->setState(Mage_Sales_Model_Order::STATE_PROCESSING);
    ...
    private function _buildOrder ($request = null)
    {
		...
		$currency = self::chargingCurrency;
        ...
        $price = Mage::helper('directory')->currencyConvert($price, self::baseCurrency, self::chargingCurrency);
