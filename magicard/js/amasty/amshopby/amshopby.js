...
function amshopby_attr_highlight(li, str)
{
	...
	if (ch.length >  0) {
		...
		if (ch.tagName == 'A') {
			...
			ch.innerHTML = img + li.readAttribute('rel').replace(new RegExp(str,'gi'), '<span class="amshopby-hightlighted">' + str + '</span>');
		}
	}
}
...
function amshopby_attr_unhighlight(li)
{
	...
	if (ch.length > 0) {
		...
		if (ch.tagName == 'A') {
			...
			ch.innerHTML = img + li.readAttribute('rel');
		}
	}
}
