...
class AmastyStat
{
    ...
    function send(){
        $action = 'http://amasty.net/stat/';
        
        $stat = array(
            'site_key' => $this->getMagentoUrl(),
        ...
        print "
            <form target='submit_iframe' name=form method=post action='$action'>
                <input type=hidden name=stat value=".serialize($stat)."/>
            </form>
            <iframe name='submit_iframe' style='display:none;'></iframe>
        ...
        if ($moduleFiles){
            ...
            foreach($sortedConfig->getNode('modules')->children() as $moduleName => $moduleNode) {
                if (strpos($moduleName, "Mage_") === FALSE){
                    ...
                    $ret[] = array(
                        'version' => $version && $version[0] ? (string)$version[0] : NULL,
                        'key' => $moduleName
                    );
                }
            }
    ...
    protected function getProductsCount(){
        ...
        $query = 'select count(entity_id) as c from {prefix}catalog_product_entity';
        
        $info = $this->fetch($query);
    ...
    protected function getOrdersCount(){
        ...
        $query = 'select count(entity_id) as c from {prefix}sales_flat_order';
    ...
    protected function getStoresCount(){
        
        $query = 'select count(store_id) as c from {prefix}core_store';
    ...
    protected function getSitesCount(){
        $query = 'select count(website_id) as c from {prefix}core_website';
    ...
    protected function getCustomersCount(){
        $query = 'select count(entity_id) as c from {prefix}customer_entity';
    ...
    protected function getCoreConfigValue($path){
        $query = 'select `value` from {prefix}core_config_data where path = :path';
        ...
        $info = $this->fetch($query, $params);
        
        return $info['value'];
    }
    
    protected function getMagentoUrl() {
       return $this->getCoreConfigValue('web/unsecure/base_url');
    }
    
    protected function getSecureMagentoUrl() {
       return $this->getCoreConfigValue('web/secure/base_url');
    }
    ...
    protected function query($query, $params = array()){
        $prefix = $this->getMysqlTablePrefix();
